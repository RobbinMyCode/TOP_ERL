...
def assign_env_to_cpu(num_env, envs, cpu_ids):
    if cpu_ids is not None and num_env > 1:
        assert len(cpu_ids) >= num_env, \
            (f"The number of cpu cores ({len(cpu_ids)}) should be >= "
             "the num of environments ({num_env}).")
...
class RLExperiment:
    ...
    AbstractIterativeExperiment],
        ...
        # else:  # only for profiling purpose
        #     self.cw.add_logger(WandBLogger())
        self.cw.run()

    @staticmethod
    def manage_code_copy_path(rep_config):
        # path: the original path from the experiment config
        # exp_path: an alias of path, otherwise meaningless
        # _basic_path: path for calling "sbatch path/to/sbatch.sh", don't change

        # Path for logging, relative to the current working directory
        rep_config["log_path"] = os.path.join("../../", rep_config["log_path"])
        rep_config["_rep_log_path"] = os.path.join("../../",
    ...
    def _process_train_rep_config_file(self, config_obj):
        ...
        for i, rep_config in enumerate(exp_configs):
            ...
            if (is_slurm(self.cw) and "experiment_copy_auto_dst"
    ...
    def _process_test_rep_config_file(exp_configs, load_model_dir, epoch,
        ...
        exp_configs[0]["keep_training"] = keep_training
