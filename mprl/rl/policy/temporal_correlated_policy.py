...
class TemporalCorrelatedPolicy(BlackBoxPolicy):
    ...
    def sample_segments(self, require_grad, params_mean, params_L, times,
                        init_time, init_pos, init_vel, idx_in_segments,
                        use_mean=False, pos_only=False):
        """
        Given a segment-wise state, rsample an action
        Args:
            require_grad: require gradient from the samples
            ...
            init_pos: initial condition pos
            init_vel: initial condition vel
            idx_in_segments: idx of segments
            use_mean: if True, return the mean action
            pos_only: if True, return only the position
        ...
        Returns:
            ...
            Shape of smp_traj:
            [*add_dim, num_pred_pairs, num_times, num_dof * 2]

            where num_times=2 if only_boundary_points is True, else num_times is
             the number of time points of [begin, end) of each segment

        """

        num_pred_pairs = idx_in_segments.shape[0]
        ...
        # [*add_dim, num_pred_pairs, num_seg_actions - 1]
        pred_at_times = times[:, idx_in_segments[..., :-1]]

        # [*add_dim, num_pred_pairs, num_times, num_dof * 2]
        smp_traj = self.sample(require_grad=require_grad,
                               params_mean=params_mean,
                               params_L=params_L, times=pred_at_times,
                               init_time=init_time, init_pos=init_pos,
                               init_vel=init_vel, use_mean=use_mean)
        if pos_only:
            return smp_traj[..., :self.num_dof]
        else:
            return smp_traj
